...
function buildLessonL40() {
  const b = makeBuilder(LEVEL_GRAPHS[40], {
    q0: [55, 49], q1: [88, 48], q2: [56, 99], q3: [91, 100],
  });
  const steps = [];
...
export default { id: 40, label: "L40", formula: "L = {w ∈ {a,b}* / a quantidade de a é impar e a quantidade de b é impar}", desc: "",                                                            shortestWord: "ab",       regex: /^.*$/, validate: w => [...w].filter(c=>c==='a').length%2===1 && [...w].filter(c=>c==='b').length%2===1, alphabet: ['a', 'b'],             acceptedWords: ["ab","abbb","aaabbb"],     rejectedWords: ["λ","a","b"],           hint: "Parecido com a anterior, mas o estado de aceitação muda.",                                                          successMsg: "Paridade ímpar cruzada!",
    tutorials: {
      onStart: { type: 'theory', title: 'Paridade Dupla: a-ímpar e b-ímpar!', dialog: [
        'L40: aceito quando #a ímpar E #b ímpar. Estado final: q3=(í,í) no canto oposto de q0.',
        ...
        'Mesmo quadrado do L39, mas o estado de aceitação muda.',
      ] },
