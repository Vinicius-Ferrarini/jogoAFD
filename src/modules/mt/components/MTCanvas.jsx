...
export default function MTCanvas({
  canvasRef, nodes, transitions, mode, setMode, beginDrag,
  ...
  const onNodeDown = useCallback((e, node) => {
    ...
    if (mode === 'CONNECTING') {
      ...
      if (src) {
        ...
        const tx = (node.x * w) / 100, ty = (node.y * h) / 100;
  ...
  }, [isDraw, mode, connectingSource, nodes, size, deleteNode, toggleInitial, toggleFinal, setConnectingSource, beginDrag, selectedNodes, setSelectedNodes]); // eslint-disable-line react-hooks/exhaustive-deps
