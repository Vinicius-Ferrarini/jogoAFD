...
export default function FormalDescriptionModal({
  ...
  onSuccess,
  showToast,
  // Recebe a função de validação do grafo vinda do App.jsx
  // Retorna true se válido, false se inválido (com toast próprio)
  onValidateGraph,
}) {
  ...
  // Reseta tudo quando o painel abre
  useEffect(() => {
    if (isOpen) {
      setInputQ('');
      setInputSigma('');
  ...
  // ── Valida elementos (Q, Σ, q0, F) ──────────────────────────────
  const validateElements = () => {
    // 1. Revalida o grafo antes de aceitar qualquer coisa
  ...
  // ── Valida transições (tabela δ) ─────────────────────────────────
  const validateTransitions = () => {
    // 2. Revalida o grafo novamente antes de aceitar a tabela
    if (onValidateGraph && !onValidateGraph()) {
      // Grafo mudou: reseta a seção de elementos para forçar repreenchimento
      setAreElementsValid(false);
      setParsedQ([]);
      setParsedSigma([]);
      setTransitionTableData({});
    ...
    for (const q of parsedQ) {
      for (const s of parsedSigma) {
        ...
        // Compara usando label visível do nó destino
