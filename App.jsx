// AutoQuest — App.jsx v2.0
// Correções: nomes duplicados (uid interno), revalidação antes da tabela,
// bug de tradução do Chrome, otimizações de perf, Maurílio fixo, simulação passo a passo
    ...
    <div className="sim-modal-overlay" onClick={onClose}>
      <div className="sim-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="sim-controls">
          <button className="sim-nav-btn" onClick={() => setStepIdx(i => Math.max(0, i - 1))} disabled={stepIdx === 0}>◀ Anterior</button>
          <button className="sim-nav-btn" onClick={() => setStepIdx(i => Math.min(steps.length - 1, i + 1))} disabled={stepIdx === steps.length - 1}>Próximo ▶</button>
        </div>
...
export default function App() {
  ...
  // nodes: { uid (interno, único), id (label visível), x, y, isInitial, isFinal }
  ...
  const loadLevel = useCallback((level) => {
    _uidCounter = 0; // reseta gerador de UIDs por fase
  ...
  // ── Validação do grafo (retorna boolean, usada pelo Modal também) ──────────
  const validateAFDSilent = useCallback((showErrors = true) => {
    ...
    // Simula contra todas as palavras de teste
  ...
  const handlePointerDownCanvas = useCallback((e) => {
    ...
    if (interactionMode === 'ADD_NODE') {
      ...
      // Gera label único visível (q0, q1, ...)
      const usedLabels = new Set(nodes.map(n => n.label));
      while (usedLabels.has(`q${num}`)) num++;
      const newLabel = `q${num}`;
      setNodes(prev => [...prev, { uid: genUid(), id: newLabel, label: newLabel, x: ix, y: iy, isInitial: false, isFinal: false }]);
      setInteractionMode('IDLE');
      return;
    }
  ...
  }, [isDrawingUnlocked, interactionMode, pan, zoom, nodes]);
  ...
  const handlePointerDownNode = useCallback((e, uid) => {
    ...
    if (interactionMode === 'ERASE') {
      ...
      setTransitions(prev => prev.filter(t => t.from !== uid && t.to !== uid));
      return;
    }
    if (interactionMode === 'TOGGLE_INITIAL') {
      setNodes(prev => prev.map(n => ({ ...n, isInitial: n.uid === uid })));
      setInteractionMode('IDLE'); return;
    }
    if (interactionMode === 'TOGGLE_FINAL') {
      setNodes(prev => prev.map(n => n.uid === uid ? { ...n, isFinal: !n.isFinal } : n));
      setInteractionMode('IDLE'); return;
    }
    if (interactionMode === 'CONNECTING') {
      if (!connectingSource) {
        ...
        setTransitions(prev => [...prev, { from: connectingSource, symbol: '', to: uid }]);
  ...
  }, [isDrawingUnlocked, interactionMode, connectingSource, selectedNodes, nodes, pan, zoom]);
  ...
  }, [isPanning, dragInfo, selectionBox, nodes, selectedNodes]);

  // ── Renomear nó — previne duplicatas ──────────────────────────────────────
  // oldLabel: valor ao focar;  newLabel: valor ao sair
  const [editingNodeLabel, setEditingNodeLabel] = useState(null); // { uid, oldLabel }
  ...
  const handleNodeLabelChange = useCallback((uid, value) => {
    // Atualiza enquanto digita (sem validar ainda)
  ...
  const handleNodeLabelBlur = useCallback((uid, currentLabel) => {
    ...
    // Se vazio ou igual ao antigo, apenas normaliza
    if (!trimmed) {
    ...
    // Verifica duplicata com outro nó
    const isDuplicate = nodes.some(n => n.uid !== uid && (n.label === trimmed || n.id === trimmed));
    if (isDuplicate) {
      const old = editingNodeLabel?.oldLabel ?? `q${uid}`;
      showToast(`Já existe um estado chamado "${trimmed}". Revertendo para "${old}".`, 'error');
      setNodes(prev => prev.map(n => n.uid === uid ? { ...n, label: old, id: old } : n));
      setEditingNodeLabel(null);
      return;
    }

    // Propaga renomeação nas transições (via uid, seguro)
    setNodes(prev => prev.map(n => n.uid === uid ? { ...n, label: trimmed, id: trimmed } : n));
    setEditingNodeLabel(null);
  }, [nodes, editingNodeLabel, showToast]);

  // ── Símbolo nas transições ────────────────────────────────────────────────
  const handleSymbolChange = useCallback((idx, val) => {
    setTransitions(prev => { const t = [...prev]; t[idx] = { ...t[idx], symbol: val }; return t; });
  }, []);
  ...
  const handleTransitionClick = useCallback((idx) => {
    ...
    if (interactionMode === 'ERASE') {
      ...
      setTransitions(prev => { const t = [...prev]; t[idx] = { ...t[idx], symbol: selectedSymbolCard }; return t; });
      setSelectedSymbolCard(null);
    }
  }, [isDrawingUnlocked, interactionMode, selectedSymbolCard]);
    ...
    <div className="workspace-wrapper">
      ...
      <div className="workspace">
          ...
          {isDrawingUnlocked && (
            <div style={{ position:'absolute', top:12, right:12, display:'flex', gap:4, zIndex:10,
              ...
              <button onClick={() => setZoom(z => Math.max(0.15, z-0.25))}
                style={{ fontWeight:'bold', width:26, cursor:'pointer', border:'none', background:'transparent', fontSize:18 }}>−</button>
        ...
        <aside className="test-panel">
          ...
          {isDrawingUnlocked && (
            <button className="simulate-btn" onClick={openSimulation}>🔬 Simular Passo a Passo</button>
          )}
      ...
      {/* ── HUD Maurílio (balão acima, personagem fixo) ── */}
      {isDrawingUnlocked && (
